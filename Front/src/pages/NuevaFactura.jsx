...
export default function NuevaFactura() {
  ...
  const plazosCredito = useMemo(() => plazos.filter(p => p.tipoId === 1 || p.tipo_id === 1), [plazos])
  const plazoLabel = useMemo(() => generarEtiquetaPlazo(modalidad, cantCuotas, tipoVenc, diasIrreg), [modalidad, cantCuotas, tipoVenc, diasIrreg])

  const updateItem = (idx, key, val) => { setItems(p => p.map((it, i) => i===idx ? {...it,[key]:val} : it)); setShowCuotas(false) }
  const removeItem = (idx) => { setItems(p => p.filter((_,i) => i!==idx)); setShowCuotas(false) }
  const addItem = () => setItems(p => [...p, { cod:'', desc:'', precio:0, iva:10, desc_pct:0, cantidad:1 }])
  const autocompleteItem = (idx, prod) => {
    setItems(p => p.map((it, i) => i===idx ? {...it, cod:prod.cod, desc:prod.desc, precio:prod.precio, iva:prod.iva, producto_id:prod.producto_id} : it))
    setShowCuotas(false)
  }
  ...
  const handlePlazo = (plazoId) => {
    const p = plazos.find(pl => pl.id === parseInt(plazoId))
    if (!p) return
    setPlazoSeleccionado(p)
    setCantCuotas(p.cuotas)
    const irregular = p.irregular || p.plazo_detalles?.length > 0
    setTipoVenc(irregular ? 'irregular' : 'regular')
    const detalles = p.plazoDetalles || p.plazo_detalles || []
    setDiasIrreg(irregular && detalles.length > 0
      ? detalles.sort((a,b) => a.cuota - b.cuota).map(d => d.dias)
      : Array.from({ length: p.cuotas }, (_,i) => (i+1)*30))
    setShowCuotas(false)
  }

  const genPreview = useCallback(() => {
    if (totals.total <= 0) { toast.error('Agregá ítems con precio'); return }
    if (!header.clienteId) { toast.error('Seleccioná un cliente'); return }
    if (modalidad==='CR' && !plazoSeleccionado) { toast.error('Seleccioná un plazo'); return }

    setCuotasPreview(modalidad==='CO'
      ? [{ numero:'"1/1"', importe:totals.total, vence:new Date(header.fechaFactura+'T12:00:00'), cobrado:0 }]
      : generarCuotas(totals.total, cantCuotas, tipoVenc, diasIrreg, new Date(header.fechaFactura+'T12:00:00')))
    setShowCuotas(true)
    toast.success('Vista previa generada')
  }, [totals.total, header.clienteId, header.fechaFactura, modalidad, plazoSeleccionado, cantCuotas, tipoVenc, diasIrreg])
  ...
  const guardar = async () => {
    ...
    try {
      const plazoId = modalidad==='CO'
        ? (plazos.find(p => p.tipoId===0 || p.tipo_id===0)?.id || null)
  ...
  if (cargando) return <div className="text-center py-10 text-gray-400">Cargando datos...</div>

  return (
    <div>
      <CabeceraFactura header={header} setHeader={setHeader} totals={totals} clientes={clientes} onModalidadChange={handleModalidad} />
      <DetalleItems items={computedItems} onUpdate={updateItem} onRemove={removeItem} onAdd={addItem} onAutocomplete={autocompleteItem} totals={totals} />
      ...
      <div className={`grid gap-3.5 ${showCuotas ? 'lg:grid-cols-2' : 'grid-cols-1'}`}>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
          ...
          <div className="grid grid-cols-2 gap-2 mb-3.5">
            {[{v:'CO',l:'Contado (CO)',d:'Pago único'},{v:'CR',l:'Crédito (CR)',d:'Cuotas'}].map(m => (
              <button key={m.v} onClick={() => handleModalidad(m.v)}
                className={`p-2.5 rounded-lg text-left transition-all ${modalidad===m.v ? 'border-2 border-brand bg-brand-light' : 'border border-gray-200 hover:border-gray-300'}`}>
          ...
          {modalidad==='CR' && (
            <div className="bg-gray-50 rounded-lg p-3.5 border border-gray-200 mb-3.5">
              ...
              <select value={plazoSeleccionado?.id||''} onChange={e => handlePlazo(e.target.value)}
                ...
                {plazosCredito.map(p => <option key={p.id} value={p.id}>{p.plazo}</option>)}
              </select>
              {plazoSeleccionado && (
                <div className="mt-2 px-3 py-1.5 bg-brand-light rounded border border-brand/20">
                  <span className="text-xs text-brand-dark font-semibold">
                    {plazoSeleccionado.cuotas} cuota{plazoSeleccionado.cuotas>1?'s':''} — {plazoSeleccionado.plazo}
                  </span>
                </div>
              )}
            </div>
        ...
        {showCuotas && (
          <TablaCuotas cuotas={cuotasPreview} tipo={header.tipoDoc}
            cliente={cliente.nombre||''} numero={`${header.factNum1}-${header.factNum2}-${header.factNum3}`}
            fecha={header.fechaFactura} moneda={header.moneda} plazoLabel={plazoLabel} />
        )}
      </div>
    </div>
  )
